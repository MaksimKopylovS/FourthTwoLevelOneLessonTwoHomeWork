...
ALTER TABLE `geodata`.`_countries` 
ADD INDEX `In_title` (`title` ASC) VISIBLE;
;

-- Редактирование таблицы rigion 
...
ALTER TABLE `geodata`.`_regions` 
ADD INDEX `In_titile` (`title` ASC) VISIBLE;
;

ALTER TABLE `geodata`.`_regions` 
ADD INDEX `Fk_countriesID_idx` (`country_id` ASC) VISIBLE;
;
ALTER TABLE `geodata`.`_regions` 
ADD CONSTRAINT `Fk_countriesID`
  FOREIGN KEY (`country_id`)
  REFERENCES `geodata`.`_countries` (`id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;

  -- Редактирование таблицы cities
